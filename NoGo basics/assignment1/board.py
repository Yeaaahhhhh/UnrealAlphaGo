...
from gtp_connection import captureOrNot
...
class GoBoard(object):
    ...
    def has_liberty(self, block: np.ndarray) -> bool:
        """
    ...
    def block_of(self, stone: GO_POINT) -> np.ndarray:
        """
    ...
    def _detect_and_process_capture(self, nb_point: GO_POINT) -> GO_POINT:
        ...
        opp_block = self.block_of(nb_point)
        if not self.has_liberty(opp_block):
    ...
    def play_move(self, point: GO_POINT, color: GO_COLOR) -> bool:
        ...
        for nb in neighbors:
            if self.board[nb] == opp_color:
                ...
                if single_capture != NO_POINT: #capture illegal

                    #single_captures.append(single_capture)
                    return False
        block = self.block_of(point)
        if not self.has_liberty(block):  # undo suicide move
    ...
    def neighbors(self, point: GO_POINT) -> List:
