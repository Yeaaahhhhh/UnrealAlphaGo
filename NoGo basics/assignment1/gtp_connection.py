...
class GtpConnection:
    ...
    def gogui_rules_legal_moves_cmd(self, args):
        ...
        for i in move:
            ...
            legalMoveList.append(format_point(coord))
        sortedLegalMoveList = ' '.join(sorted(legalMoveList))
        self.respond(sortedLegalMoveList)
        return
            ...
            if not self.board.play_move(move, color):
                ...
                for nb in nbs:
                    if self.board.board[nb] == opponent(color):
                        if not self.board.has_liberty(self.board.block_of(nb)):
